...
var slotScrollAndStop = new Audio(
  "/sounds/Slots_Classic/slotScrollAndStop.mp3"
);
...
conn.start().then(function () {
  $(".spin-button").click(async function () {
    ...
    if (result.isSuccess === false) {
      new Snack({
        message: `Error: ${result.errorMessage.toString()}`,
        duration: 3000,
        type: `error`,
      });

      return console.error(result.errorMessage.toString());
    }
    modalOn.play();
    gambleSound.play();
    ...
    Object.values(rollSlots).forEach((item) => {
      item.setAttribute(
        "src",
        `${IMG_PATH}animation-${item.getAttribute("data-slot")}.gif`
      );
    });
    ...
    let showResultsInterval = setInterval(() => {
      document
        ...
        .setAttribute("src", `${IMG_PATH}${result.symbols[i]}.png`);
      i++;
      if (i === 3) {
