...
crashConn.start().then(function () {
  ...
  crashConn.stream("StreamCrashInfo").subscribe({
    next: function (data) {
      ...
      if (JSON.stringify(data.players) !== JSON.stringify(playersOld)) {
        // If the players list changed
      ...
      if (
        JSON.stringify(data.onlinePlayers) !== JSON.stringify(onlinePlayersOld)
      ) {
      ...
      if (playerData.autoBetting) {
        playerData.autoBetsTotal =
          document.getElementById("auto_total-bets").value;
      }
      ...
      if (playerData.isBetting) {
        if (streamData.gameStarted) {
          manualButton.textContent =
            "Cashout +" +
            (playerData.amount * streamData.multiplier).toFixed(0);
        } else {
  ...
  async function placeBet(amount) {
    let result = await crashConn
      .invoke("JoinCrash", amount)
      .catch(function (err) {
        new Snack({
  ...
  function lockBettingButtons(status) {
    if (status == true) {
      document
        .getElementById("manual-place-bet")
        .classList.add("bet-button-disabled");
      document
        .getElementById("auto-place-bet")
        .classList.add("bet-button-disabled");
    } else {
      document
        .getElementById("manual-place-bet")
        .classList.remove("bet-button-disabled");
      document
        .getElementById("auto-place-bet")
        .classList.remove("bet-button-disabled");
    }
