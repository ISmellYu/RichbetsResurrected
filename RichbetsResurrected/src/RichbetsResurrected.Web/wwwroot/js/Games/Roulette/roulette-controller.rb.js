...
conn.start().then(function () {
  ...
  conn.stream("StreamRouletteInfo").subscribe({
    next: function (data) {
      ...
      if (actualProgress < 98) {
        // Prevent the timer from visual bug.
  ...
  conn.on("EndRoulette", function (history, current) {
    ...
    if (PlayerBetHistory.includes(current.color)) {
      // Play correct sound if the player won.
      playSound("rollWin");
    } else {
      playSound("rollEnd");
    }
  ...
  async function placeBet(color, amount) {
    let result = await conn
      .invoke("JoinRoulette", amount, color)
      .catch(function (err) {
        new Snack({
    ...
    if (result.isSuccess == true) {
      ...
      new Snack({
        message: `Bet placed on ${translatedColor}`,
        duration: 5000,
      });
    }
  }
    ...
    colors.forEach((color) => {
      ...
      if (nodes.length < 2) {
        document.querySelector(
          `.count-bets-${lowerColor}`
        ).textContent = `${nodes.length} Bet`;
      } else {
        document.querySelector(
          `.count-bets-${lowerColor}`
        ).textContent = `${nodes.length} Bets`;
      }
      ...
      document
        .querySelector(`.all-bets-${lowerColor}`)
