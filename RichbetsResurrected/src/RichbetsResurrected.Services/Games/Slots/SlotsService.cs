...
public class SlotsService : ISlotsService
{
    ...
    public SlotsService(ILifetimeScope lifetimeScope, IRichbetRepository richbetRepository,
    ...
    public async Task<SlotsSpinResult> SpinAsync(SlotsSpinRequest request, string connectionId)
    {
        ...
        if (request.Amount <= 0)
        {
            return new SlotsSpinResult {IsSuccess = false, ErrorMessage = "Bet amount must be greater than 0"};
        }
        ...
        var slotSpinResult = new SlotsSpinResult {IsSuccess = true, Symbols = generatedSymbols};

        var withdrawResult = WinHandler.Handle(generatedSymbols, request.Amount);

        _backgroundTasks.DelaySlotsWithdrawalAsync(withdrawResult,
            Convert.ToInt32(1000 * request.DelayAmountToWithdraw), request.UserId,
            connectionId);
    ...
    private SlotsWithdrawResult GetWithdrawResult(IReadOnlyList<SymbolEnum> symbolArray, int amount)
    {
        ...
        {
            if (symbolArray.Count(p => p == symbolArray[i]) != 2)
            {
                continue;
            }
        ...
        return new SlotsWithdrawResult {IsWin = false, Multiplier = null, WinAmount = null};
    }
